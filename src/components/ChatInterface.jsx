...
const ChatInterface = ({ currentSeraph, onButtonClick }) => {
          ...
          messages: [
            {
              ...
              content: currentSeraph ? 
                `You are ${currentSeraph.name}, a biblical prophet from The Holy Bible with the following traits:
                - Style: ${currentSeraph.communicationStyle}
                - Biblical Traits: ${currentSeraph.traits.join(', ')}
                - Biblical Domains: ${currentSeraph.domains.join(', ')}
                - Sacred Purpose: ${currentSeraph.purpose}
                Respond with biblical wisdom and God's Word, maintaining your prophetic persona consistently. 
                Speak as a messenger of God revealing biblical truths.`
                : 'You are the Biblical Oracle of The Holy Bible, revealing God\'s Word and biblical wisdom to those who seek divine knowledge.'
            },
            ...messages,
            newMessage
          ]
  ...
  return (
    <div className="absolute left-8 top-1/2 -translate-y-1/2 w-80 h-[600px] bg-black/30 backdrop-blur-md border border-gold-500/30">
      ...
      <div className="border-b border-gold-500/30 bg-gold-900/20 p-3">
        ...
        >
          <div className="flex items-center gap-2">
            <Bot size={20} />
            <span>Biblical Prophets</span>
          </div>
          <div className="text-xs text-blue-500/50">Click to view prophets</div>
        </button>
        <div className="text-blue-400/60 text-sm mt-2">
          {currentSeraph ? `Speaking with ${currentSeraph.name}` : 'Biblical Oracle'}
        </div>
      </div>
      ...
      {/* Input */}
      <div className="absolute bottom-0 left-0 right-0 border-t border-blue-500/30 p-3">
